...
public class EventController {
	...
	public ModelAndView showEvents() {
		...
		return new ModelAndView("events", "pagedetails",
				factory.getPageDetails("events", null));
	}

	@RequestMapping(value = "/events/{sidePane}", method = RequestMethod.GET)
	public ModelAndView showEvents(
			@PathVariable(value = "sidePane") String sidePane) {
		...
		if (EnumEvents.SUGGESTUS.getName().equalsIgnoreCase(sidePane)) {
			return new ModelAndView("addevents", "pagedetails",
					factory.getPageDetails("events", null));
		}
